--[[
	...
	Revision: $Id: GatherConfig.lua 927 2011-10-05 01:25:05Z kjasi $
...
Gatherer_RegisterRevision("$URL: http://svn.norganna.org/gatherer/trunk/Gatherer/GatherConfig.lua $", "$Rev: 927 $")
...
function MakeGuiConfig()
	...
	curpos = gui:GetLast(id)
	gui:AddControl(id, "Checkbox", 0.5, 1, "show.minimap.arch", _trL("Show Archaeology nodes"))
	gui:SetLast(id, curpos)
	gui:AddControl(id, "Checkbox", 0.85, 1, "show.minimap.arch.all", _trL("All"))
	gui:AddControl(id, "Checkbox", 0.5, 2, "show.minimap.arch.onlyifskilled", _trL("Archaeologists Only"))
	...
	curpos = gui:GetLast(id)
	gui:AddControl(id, "Checkbox", 0.5, 1, "show.mainmap.arch", _trL("Show Archaeology nodes"))
	gui:SetLast(id, curpos)
	gui:AddControl(id, "Checkbox", 0.85, 1, "show.mainmap.arch.all", _trL("All"))
	gui:AddControl(id, "Checkbox", 0.5, 2, "show.mainmap.arch.onlyifskilled", _trL("Archaeologists Only"))
	...
	id = gui:AddTab(_trL("Archaeology"))
	gui:MakeScrollable(id)
	gui:AddControl(id, "Header",     0,    _trL("Archaeology filter options"))
	gui:AddControl(id, "Subhead",    0,    _trL("Archaeology Fragment nodes to track"))
	local options = {}
	local list = itemLists.arch
	table.sort(list, entrySort)
	for pos, arch in pairs(list) do
		table.insert(options, { "show.arch."..arch[1], arch[2] })
	end
	gui:ColumnCheckboxes(id, 1, options)
